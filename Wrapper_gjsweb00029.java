...
/**
 * Created by peng.ni on 14-6-24.
 */

public class Wrapper_gjsweb00029 implements QunarCrawler {
    private static final Logger logger = LoggerFactory.getLogger("CrawlerLog");
    private static final String CODEBASE = "gjsweb00029";
    ...
    public String getHtml(FlightSearchParam param) {
        ...
        try{
            ...
            String forQuery = String.format("tabid=3582&fromDt=%s&fromTm=1100&toDt=%s&toTm=1100&rt=true&ad=1&se=0&ch=0&class=1&infl=0&infs=0&airpref=&preftyp=1&searchflxdt=false&IsNS=false&searchflxarpt=false&from=%s&to=%s",
                    param.getDepDate(),param.getRetDate(),param.getDep(),param.getArr());
            ...
            if (post.getStatusCode() == HttpStatus.SC_MOVED_PERMANENTLY ||
                ...
                if (locationHeader != null) {
                    ...
                    // get = new QFGetMethod("http://www.onetravel.com/Default.aspx?tabid=2000&t=f&FSS=&sid=1");
                    ...
                    String forQuery2=String.format("http://www.onetravel.com/Default.aspx?tabid=1919&sid=1&oa=%s&da=%s&adt=1&chd=0&snr=0&infl=0&infs=0&dd=%s&rd=%s&tt=ROUNDTRIP",param.getDep(),param.getArr(),param.getDepDate(),param.getRetDate());
    ...
    public RoundTripFlightInfo getDetailInfo(String data,FlightSearchParam param) //提取具体航班字段
    {
        RoundTripFlightInfo roundTripFlightInfo=new RoundTripFlightInfo();
        String depYear=param.getDepDate().split("-")[0];
        String depMonth=param.getDepDate().split("-")[1];
        String arrYear=param.getRetDate().split("-")[0];
        String arrMonth=param.getRetDate().split("-")[1];
        ...
        String[] airLinePartRaw= data.split("listSep");//拆分成往返的两部分
        ...
        List<FlightSegement> info= getSegementList(airLinePartRaw[0],depMonth,depYear);
        List<FlightSegement> retInfo= getSegementList(airLinePartRaw[1],arrMonth,arrYear);

        List<String> retFlightInfo=Lists.newArrayList();//获得返程航班号
        for(FlightSegement fs:retInfo)
        {
            retFlightInfo.add(fs.getFlightno());
        }
        ...
        roundTripFlightInfo.setRetflightno(retFlightInfo);
        roundTripFlightInfo.setRetdepdate(retDepDate);

        return roundTripFlightInfo;
    }

    public List<FlightSegement> getSegementList(String data,String month,String year)
    {
        //大致分割网页信息
    ...
    public static void main(String[] args)
    {
        ...
        f.setArr("ABR");
        f.setDep("ADK");
        f.setDepDate("2014-10-01");
        f.setRetDate("2014-11-26");
        Wrapper_gjsweb00029 wr=new Wrapper_gjsweb00029();
